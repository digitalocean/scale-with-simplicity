...
output "check_direct_connect_bgp_command" {
  value = "aws directconnect describe-virtual-interfaces --region ${var.aws_region} --virtual-interface-id ${local.aws_connection.vif_id} --query 'virtualInterfaces[*].{ID:virtualInterfaceId, BGPStatus:bgpPeers[0].bgpStatus}' --output table"
}
